...
async fn main() {
    ...
    // Configuration
    // On récupère l'URL de Neon depuis les variables d'environnement
    let database_url = std::env::var("DATABASE_URL")
        .expect("DATABASE_URL must be set (ex: Neon connection string)");
    
    let jwt_secret = std::env::var("JWT_SECRET")
        .unwrap_or_else(|_| "super_secret_jwt_key_change_in_production".to_string());

    // Connexion à PostgreSQL (Neon nécessite SSL par défaut)
    // PgPoolOptions gère automatiquement le SSL si l'URL contient sslmode=require
    let pool = PgPoolOptions::new()
        .max_connections(10)
        .connect(&database_url)
        .await
        .expect("Failed to connect to NeonDB. Check your connection string and SSL mode.");

    println!("✅ Connected to NeonDB Cloud");
    ...
    // Routes
    let app = Router::new()
        .route("/", get(|| async { "Welcome to the Hello World API!" })) // Add this line
        // Health
